...
function computeTechnicalScore(metrics, item = {}) {
  ...
  const weightedTrend = (trendScore / 25) * 20;
  const weightedMomentum = (momentumScore / 20) * 15;
  const weightedPosition = (positionScore / 15) * 10;
  const weightedLongTerm = (longTermScore / 30) * 20;
  const weightedSetup = (Math.min(8, setupScore) / 8) * 5;
  const weightedRs = normalizeRange(rsRating, 1, 99, 30);
  const penalty = structuralPenalty(metrics, item);
  const rawScore = weightedTrend + weightedMomentum + weightedPosition + weightedLongTerm + weightedSetup + weightedRs - penalty;
